...
export default function ContactErrorBoundary({
  error,
  reset,
}: ContactErrorProps): React.JSX.Element {
  const t = useTranslations("error");

  useEffect(() => {
    console.error("[ContactErrorBoundary caught route exception]:", error);
  }, [error]);
      ...
      <div className="max-w-md w-full p-8 rounded-2xl border border-border bg-card/80 shadow-lg backdrop-blur-xs">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            type="button"
            onClick={() => reset()}
            className="w-full sm:w-auto inline-flex items-center justify-center px-5 py-2.5 rounded-lg font-medium text-sm bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 cursor-pointer"
          >
            {t("retry")}
          </button>
          <Link

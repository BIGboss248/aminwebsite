...
import { DohMethodologyTooltip } from "./DohMethodologyTooltip";
import type { DohToolHeaderProps } from "./DohToolHeader.types";

/**
 * Header Section for the DoH Prober Tool (`DohToolHeader`).
 */
export function DohToolHeader({
  locale = "en",
  eyebrow,
  badge,
  title,
  ...
  className = "",
  ...rest
}: DohToolHeaderProps): React.JSX.Element {
  const tHeader = useTranslations("lab.doh_prober.header");
  ...
  return (
    <header
      aria-labelledby="doh-prober-heading"
      className={cn(
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <DohMethodologyTooltip locale={locale} />
      </div>
    </header>
  );
}

export default DohToolHeader;

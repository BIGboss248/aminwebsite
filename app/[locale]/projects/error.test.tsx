...
import ProjectsErrorBoundary from "./error";
...
describe("Projects ErrorBoundary Component", () => {
  ...
  it("renders error UI with title, description, and calls reset when clicked", () => {
    ...
    const testError = new Error("Test anomaly in projects route");

    render(<ProjectsErrorBoundary error={testError} reset={mockReset} />);
  ...
  it("renders error digest when provided", () => {
    ...
    const testError = Object.assign(new Error("Test error with digest"), {
      digest: "PROJECTS_ERR_1092",
    });

    render(<ProjectsErrorBoundary error={testError} reset={mockReset} />);

    expect(screen.getByText("PROJECTS_ERR_1092")).toBeInTheDocument();
  });
});

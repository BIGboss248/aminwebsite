...
flowchart TD
    ...
    Step2 --> Step3["Step 3: Benchmark & Inspiration Mining<br/>(StitchMCP, Browser MCPs, Benchmark Sites & Proactive generate_image)"]
...
└── [generated assets...]   # Any bespoke images, backdrops, or icons created via generate_image
```
...
### Step 3: Benchmark, Inspiration & Proactive Image Generation

To defeat the "Curse of the White Page" and ground the design in proven patterns:

#### 1. Proactive Visual Asset Generation (`generate_image`)
...
- **`StitchMCP`** (`generate_screen_from_text`, `create_project`, `generate_variants`): Google Stitch screen scaffolding.
- **`playwright`** & **`chrome-devtools-mcp`**: Real-time browser automation to inspect live benchmark sites, extract CSS styles, and capture screenshots into artifacts.
- **`codebase-memory`**: Inspects prior design ADRs and documented architecture patterns.

#### 3. Curated Inspiration Benchmarks

- **Mobbin** (`mobbin.com`), **Godly** (`godly.website`), **Minimal Gallery** (`minimal.gallery`), **Land-book** (`land-book.com`), **Coolors for Tailwind** (`coolors.co/tailwind`), **Fontjoy** (`fontjoy.com`), **Spline** (`spline.design`).
...
2. **Visual Hierarchy & Spatial Flow:** 1st, 2nd, and 3rd focal points.
3. **Layout Grid & Breakpoints:** Stacking behavior for mobile (<768px), tablet (768-1024px), desktop (>=1024px).
4. **Design Tokens & Color Mapping:** Light and dark values from `docs/project.json`.
5. **Typography Scale (Per Locale):** Typographic sizing, weights, and line heights for each locale defined in `docs/project.json`.
6. **Interaction States Matrix:** Default, hover, active, focus rings, loading states.
7. **Bidirectional (RTL) Adaptations:** Mirroring rules, code preservation, directional icon flips (if RTL is supported).
8. **Accessibility & Ergonomics:** WCAG AA/AAA contrast ratios, touch targets ($\ge 44\text{px}$), focus rings.
...
- [ ] `design-spec.md` details hierarchy, tokens, per-locale typography, and states.

...
description: >
  Define a role to play as, success criteria (GOAL), sub-task list, constraints, output format, realistic example, AND explicit failure conditions in a single structured contract. The agent treats it as a spec, not a suggestion. Triggers on "prompt contract", "write a contract", "define success criteria", "failure conditions", "spec this out", or /prompt-contracts. Also triggers on phrases like "what does done look like", "define the contract", "structured spec", or "goal constraints format failure".
allowed-tools: Read, Grep, Glob, Bash, Task, Write, Edit, AskUserQuestion
---

# Prompt Contracts

Define a structured contract before implementation covering **ROLE** (AI persona/expertise), **GOAL** (quantifiable success), **TASKS** (sub-task list), **CONSTRAINTS** (hard limits), **FORMAT** (exact output shape), **EXAMPLE** (exemplar of task well done), and **FAILURE** (explicit conditions that mean "not done"). The agent treats this as an engineering spec with zero ambiguity about what "done" means.
...
**If the user provides a contract:** Parse it into all required sections and proceed to validation (step 3).
...
ROLE: [Persona or domain expertise the AI should adopt, e.g., Senior Software Engineer]

GOAL: [What does success look like? Include a measurable metric.]

TASKS:
1. [Sub-task 1: Initial setup / research]
2. [Sub-task 2: Core implementation step]
3. [Sub-task 3: Testing & verification]
...
EXAMPLE:
- [Concrete snippet, sample input/output, or response reference of what well-done looks like]
...
#### Writing good ROLE statements:
- Specify expertise domain & level: "Senior Node.js & TypeScript Architect" rather than generic "Developer"
- Define engineering mindset: "Focus on clean code, performance, safety, and defensive programming"
...
#### Writing good TASKS lists:
- Break complex tasks down into small, ordered, clear sub-tasks
- Make each task step discrete and mechanically verifiable
- Sequence steps logically (e.g. data schema -> business logic -> API/UI -> test suite)
...
#### Writing good EXAMPLE sections:
- Provide an ideal input/output pair, code block, or response sample demonstrating high quality
- Show exact expected schemas, patterns, or styling standards to reduce ambiguity
...
- **Complete** — all sections (ROLE, GOAL, TASKS, CONSTRAINTS, FORMAT, EXAMPLE, FAILURE) filled out
- **Consistent** — CONSTRAINTS and TASKS don't contradict GOAL
- **Testable** — every task step and FAILURE condition can be mechanically verified
- **Scoped** — GOAL and TASKS are achievable within the CONSTRAINTS
...
- **ROLE**: The persona and engineering standards you uphold
- **GOAL**: The target outcome and metric you are optimizing for
- **TASKS**: The ordered checklist of action steps to execute
- **CONSTRAINTS**: Boundaries you cannot cross
- **FORMAT**: Exact shape of your output
- **EXAMPLE**: The quality reference to match
- **FAILURE**: Conditions you must actively prevent

As you implement, mentally check each task and FAILURE condition. If you catch yourself about to violate one, stop and fix it before moving on.

### 5. Self-verify against FAILURE conditions

Before delivering, run through every FAILURE condition and task step as a checklist:

```markdown
## Contract Verification

- [ ] ROLE adopted: {role and standards applied}
- [ ] GOAL metric met: {evidence}
- [ ] TASKS completed: {all sub-tasks checked off}
- [ ] All CONSTRAINTS respected: {confirmation}
- [ ] FORMAT matches spec: {confirmation}
- [ ] EXAMPLE standard met: {confirmation against sample output}
...
TASKS: ✓ {all sub-tasks completed}
CONSTRAINTS: ✓ {all respected}
FORMAT: ✓ {matches spec}
EXAMPLE: ✓ {matched example standard}
...
### API endpoint:
```
ROLE: Senior Backend Engineer specializing in Node.js and RESTful API architecture.

GOAL: GET /users endpoint returning paginated results, handling 10K concurrent connections.

TASKS:
1. Define TypeScript interfaces for request query params and response pagination schema.
2. Build input validation middleware for limit and offset parameters.
3. Write Prisma database query with limit/offset pagination.
4. Implement route controller returning JSON payload.
5. Create 5 unit tests for success, empty set, and error edge cases.
...
```json
{
  "data": [{ "id": "usr_101", "name": "Alex", "email": "alex@example.com" }],
...
### React component:
```
ROLE: Lead Frontend Engineer focusing on React 18, accessibility, and high-performance UI components.

GOAL: Reusable DataTable component supporting sort, filter, and pagination for up to 10K rows without lag.

TASKS:
1. Define generic TypeScript props interface for row data and column configurations.
2. Implement virtualized list windowing for smooth 60fps rendering.
3. Implement debounced filter input and multi-type sort handlers.
4. Add empty state and loading skeleton UI states.
5. Create Storybook stories and unit tests for user interactions.
